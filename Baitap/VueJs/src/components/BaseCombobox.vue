<template>
    <div v-click-outside="closeDropdown">
        <BaseInput :tabindex="tabindex" @showDropdown="isShow = true" @updateInput="updateInput"
            ...
            :contentPlHolder="contentPlHolder" validType="combobox" :isDisabled="isDisabledInput"/>
        ...
        <div ref="select" style="z-index:3" v-show="isShow" class="select-custom" :class="directionDrop">
            <div v-for="(option,index) in data" :key="index" @click="evtMouseChoosingOption(option['content'])"
                ...
                <div class="option-icon"><img :class="[ option.isChosen ?'show-image': '']"
                        src="../assets/icon/check.png" alt=""></div>
                <div class="option-content none-pointer">{{option.content}}</div>
            </div>
        </div>

        <div @click="toggleDropdown" :class="{'rotate-oppositeX':isShow}" class="icon-container"><i
                class="fas fa-chevron-down icon-input"></i></div>
    </div>
...
<script>
    ...
    export default {
        props: ['typeDataPaging','contentPlHolder', 'type','dataType', 'isDisabledInput' ,'errorContent', 'isSearching', 'tabindex', 'directionDrop'],
        async created() {
            ...
            data() {
                ...
                if (this.$refs.select.classList.contains("dropup")) {
                    if(this.data.length <= 4){
                        this.$refs.select.style.top = -(15 + 40*this.data.length) + "px";
                    }else{
        ...
        methods: {
            ...
            updateInput(input) {
                ...
                this.changeStatusOptions(input);
            ...
            filterOption(content) {
                let obj = {}
                // Thêm option chọn tất cả khi các combobox để tìm kiếm
                if (this.isSearching == "true") {
                    ...
                    // Lọc các option có chứa nội dung trong input(Không phân biệt hoa, thường)
                    this.data = [obj, ...this.$store.state.data[this.dataType]].filter(element => element.content
                        .toLowerCase()
                        .search(content.toLowerCase()) != -1 || content == "")

                } else {
                    // Lọc các option có chứa nội dung trong input của combobox không tìm kiếm(Không phân biệt hoa, thường)
                    this.data = [...this.$store.state.data[this.dataType]].filter(element => element.content
                        .toLowerCase()
                        .search(content.toLowerCase()) != -1 || content == "")
                }
                // Chuyển tất cả trạng thái được chọn và được trỏ về false
                this.data.forEach(element => element.isChosen = false)
                this.data.forEach(element => element.isPointed = false)
