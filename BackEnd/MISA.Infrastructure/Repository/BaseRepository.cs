...
namespace MISA.Infrastructure.Repository
{
    public class BaseRepository<MISAEntity> : IBaseRepository<MISAEntity> where MISAEntity:BaseEntity
    {
        ...
        public BaseRepository(IConfiguration configuration)
        {
            ...
            _connectionString = _configuration.GetConnectionString("MISACukCukConnectionString"); // Chuỗi kết nối
            // Kết nối database
        ...
        public virtual int InsertMISAEntity(List<MISAEntity> misaEntityList)
        {
            DapperPlusManager.Entity<MISAEntity>().Table(_tableName);
            var rowAffects = 0;
            using (_dbConnection)
            {
                
                // Gửi câu truy vấn lên db và nhận lại kết quả
                if(misaEntityList.Count > 1)
                {
                    _dbConnection.BulkInsert(misaEntityList);
                    rowAffects = misaEntityList.Count();
                }
                else
                {
                    _dbConnection.Open();
                    ...
                    rowAffects = _dbConnection.Execute(sqlCommand, misaEntityList, commandType: CommandType.StoredProcedure);
                }
            }
            return rowAffects;
        }
