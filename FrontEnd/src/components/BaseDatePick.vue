<template>
    <div>
        <date-picker @click.native="toggleCalendar($event)" :disabled-date="isFutureDate"
            ...
            @keydown.enter.native="checkBlur($event)" :format="format" :clearable="false"
            v-click-outside="hideCalendar" @mouseover.native="isDisplayToolTip=true" @mouseleave.native="isDisplayToolTip=false"
            :input-attr="{
                tabindex : tabindex,
                maxlength: maxlength,
...
<script>
    ...
    export default {
        created() {
            this.errorContent = this.resource[this.$browserLocale]['management']['entity']['Employee'][
                'dictionaryError'
            ][this.name].invalid;
        },
        mounted() {
            ...
            inputDate.addEventListener('mousedown', () => {
                ...
                if(datePickContent)
                    datePickContent.classList.remove('display-none');
        ...
        methods: {
            ...
            toggleCalendar($event) {
                ...
                if ($event.target.classList.length == 0 || $event.target.classList.contains('mx-icon-calendar')) {
                    ...
                    if(this.isToggleCalendar){
                        ...
                        datePickContent.classList.add('display-none');
                    }
            ...
            formatDateInput($event) {
                ...
                if (dateInput.value) {
                    ...
                    console.log(dateInput.value,newDate);
                }
            },
            /**
